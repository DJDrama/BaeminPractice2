...
import com.dj.baeminpractice2.databinding.FragmentBaeminServiceTermBinding
...
class BaeminServiceTermFragment : Fragment(R.layout.fragment_baemin_service_term),
    View.OnClickListener {

    private val viewModel by viewModels<BaeminServiceTermViewModel> {
        BaeminServiceTermViewModelFactory(
            getBaeminServiceTermsUseCase = GetBaeminServiceTermsUseCase(
    ...
    private var _binding: FragmentBaeminServiceTermBinding? = null
    private val binding: FragmentBaeminServiceTermBinding
        get() = _binding!!
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        _binding = FragmentBaeminServiceTermBinding.bind(view)
    ...
    private fun subscribeToFragmentResultListeners() {
        parentFragment?.parentFragmentManager?.setFragmentResultListener(
            ...
            viewModel.handleIntent(
                intent = BaeminServiceTermIntent.ShowChosenTermContent(
            ...
            repeatOnLifecycle(state = Lifecycle.State.STARTED) {
                launch {
                    viewModel.uiEvent.collect {
                        when (it) {
                            is BaeminServiceTermUiEvent.ListAllTerms -> showAllTerms(
                                terms = it.terms,
    ...
    private fun showAllTerms(terms: List<TermPresentationModel>, currentTermId: Int) {
        findNavController().navigate(
            ...
            args = bundleOf(
                ...
                BUNDLE_KEY_TERM_TYPE to TermType.BAEMIN_SERVICE_TYPE.ordinal
            )
        )
    }
    ...
    override fun onClick(p0: View?) {
        p0?.let {
            when (it) {
                mergedBinding.flChooseTermByDate -> viewModel.handleIntent(intent = BaeminServiceTermIntent.OnClickChooseTerms)
            }
        }
    }
